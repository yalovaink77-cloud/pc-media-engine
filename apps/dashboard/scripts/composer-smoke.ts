/**
 * Content composer dashboard smoke — Sprint 40.
 */
...
async function main(): Promise<void> {
  ...
  section('1 · Composer page');
  {
    ...
    assert(res.body.includes('data-testid="composer-validate-button"'), 'validate button present');
    assert(res.body.includes('href="/composer"'), 'composer nav link present');
  }

  section('2 · Validation flow');
  {
    ...
    assert(res.statusCode === 302, 'validate redirects');
    const page = await app.inject({ method: 'GET', url: res.headers.location ?? '' });
    assert(
      page.body.includes('data-testid="composer-validation-result"'),
